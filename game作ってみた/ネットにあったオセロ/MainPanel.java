...
public class MainPanel extends JPanel implements MouseListener, Observer{
	...
	*/
	...
	public void mousePressed(MouseEvent e){
		...
		x /= SIZE;
		y /= SIZE;
		//標準出力（コンソール（たぶんコマンドプロンプト）に出力）
		System.out.println("(x,y)=("+x+","+y+")");
		
		//駒を置く＆置けない場合はメッセージ
		//state.put(x,y) はその(x,y)の座標地点に置くことができるかチェックするメソッド(GameState内のメソッド)
		if( state.put(x,y) == false ){
			/*
			メッセージダイアログを表示
			JOptionPane.showMessageDialog(c, s);
			c = Containerオブジェクト
			JFrameを継承しているクラスなので、その中のContainerオブジェクトも継承している そのため、this でこのクラス内のContainerを選択できる
			s = "表示する文字列"
			*/
			JOptionPane.showMessageDialog(this, "Can't put this area!");
		}
		
		//盤面が埋まる、またはどちらかの駒が0のとき終了
		if(state.turn == (Squares * Squares) - 4){
			JOptionPane.showMessageDialog(this, "End!");
			//処理を終わらせる(変数が0だと成功して終了、変数が-1だと失敗して終了を表す)
			System.exit(0);
		}
		/*
		パスチェック
		stateクラス内のcheckPassメソッドで置くことが可能な場所があるかどうか探す
		*/
		else if( state.checkPass() == true ){
			/*
			stateのplayer変数は、1か-1かで手番を判断しているので-1を掛けることで手番を変えることができる
			*/
			state.player *= -1;
			/*
			playerのどちらもパスのときはこれ以上置けないので、終了させる
			*/
