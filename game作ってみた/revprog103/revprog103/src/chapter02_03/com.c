...
static int Com_MidSearch(Com *self, int in_depth, int in_color, int in_opponent, int in_pass, int *out_move);
...
{
	int x, y;
	int value, max = -BOARD_SIZE * BOARD_SIZE;
	int can_move = 0;
	int move;
	...
	*out_move = NOMOVE;
	...
	if (!can_move) {
		if (in_pass) {
			...
			*out_move = PASS;
			max = -Com_MidSearch(self, in_depth - 1, in_opponent, in_color, 1, &move);
		}
	}
	return max;
}
	...
	*out_move = NOMOVE;
	for (x = 0; x < BOARD_SIZE; x++) {
		for (y = 0; y < BOARD_SIZE; y++) {
			if (Board_Flip(self->Board, in_color, Board_Pos(x, y))) {
				...
				value = -Com_EndSearch(self, in_depth - 1, in_opponent, in_color, 0, &move);
				Board_Unflip(self->Board);
				if (value > max) {
					max = value;
					*out_move = Board_Pos(x, y);
	...
	if (!can_move) {
		if (in_pass) {
			...
			*out_move = PASS;
			max = -Com_EndSearch(self, in_depth, in_opponent, in_color, 1, &move);
		}
	}
	return max;
}
