========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : reversi_gui
========================================================================


AppWizard が生成した reversi_gui には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには reversi_gui を構成している各ファイルの概要説明が含まれています。
...
reversi_gui.h
    ...
    みます） をインクルードし、また CReversi_guiApp アプリケーションクラスを宣
    言します。

reversi_gui.cpp
    このファイルは CReversi_guiApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\reversi_gui.ico
    ...
    ルです。このアイコンはリソース ファイル reversi_gui.rc によってインクルード
    されます。
...
reversi_guiDoc.h, reversi_guiDoc.cpp - ドキュメント
    これらのファイルは CReversi_guiDoc クラスを含みます。特別なドキュメント デ
    ータを付け加えたり、CReversi_guiDoc::Serialize を使ったファイルのセーブや
    ロード機能をインプリメントしたいときはこのファイルを編集してください。

reversi_guiView.h, reversi_guiView.cpp - ビュー
    これらのファイルは CReversi_guiView クラスを含みます。
    CReversi_guiView オブジェクトは CReversi_guiDoc オブジェクトを表示するために
    使います。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) reversi_gui.pch 
